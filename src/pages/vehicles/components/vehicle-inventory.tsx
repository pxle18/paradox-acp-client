...
import vehicleInventoryService from "app/services/vehicle-inventory.service";
...
const VehicleInventory: React.FC = () => {
  ...
  useEffect(() => {
    vehicleInventoryService.getVehicleInventory(
      currentVehicle.id
    ).then(response => {
  ...
  const deleteItem = (slot: number): void => {
    vehicleInventoryService.deleteItemBySlot(
      currentVehicle.id,
      slot
    ).then(response => {
      if(response.data !== undefined) {
  ...
  const clearInventory = (): void => {
    vehicleInventoryService.clearInventory(
      currentVehicle.id
    ).then(response => {
      if(response.data !== undefined) {
  ...
  const requestAddItem =  async () => {
    ...
    modalService.showWithData(
      ModalType.SELECT, 
      "Item hinzufügen", 
      "Welches Item möchten Sie dem Fahrzeug hinzufügen?",
      items.data,
  ...
  const requestAddItemAmount = (input: string | boolean): void => {
    ...
    modalService.show(
      ...
      (amount) => addItem(input, amount)
    )
  }

  const addItem = (item: string | boolean, amount: string | boolean): void => {
    if (typeof item == "boolean") return;
    if (typeof amount == "boolean") return;

    vehicleInventoryService.addItem(
      currentVehicle.id,
      item,
      parseInt(amount),
    ).then(response => {
