...
class UserInventoryService {
  getUserInventory = async (id: number): Promise<Api<Inventory>> => {
    loadingService.setLoading(true);

    return await api
      .get(`player/${id}/inventory`, { headers: this.getHeaders() })
      .then(response => {
  ...
  addItem = async (id: number, item: string, amount: number): Promise<Api<CallbackMessage & any>> => {
    ...
    return await api
      .post(`player/${id}/inventory/add`, { item, amount }, { headers: this.getHeaders() })
      .then(response => {
        loadingService.setLoading(false);
  ...
  deleteItemBySlot = async (id: number, slot: number): Promise<Api<CallbackMessage>> => {
    ...
    return await api
      .post(`player/${id}/inventory/delete`, { slot }, { headers: this.getHeaders() })
      .then(response => {
        loadingService.setLoading(false);
  ...
  clearInventory = async (id: number): Promise<Api<CallbackMessage>> => {
    ...
    return await api
      .get(`player/${id}/inventory/clear`, { headers: this.getHeaders() })
      .then(response => {
        loadingService.setLoading(false);
...
export default new UserInventoryService();